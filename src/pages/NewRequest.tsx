import { ArrowLeft, Check, MapPin, Search, CheckCircle, Image as ImageIcon, CloudUpload, X, Info, ArrowRight, LocateFixed, Plus, Minus, Loader2 } from 'lucide-react';
...
export function NewRequest() {
  ...
  const handleSubmit = async () => {
    ...
    // Auto-formatting the date to match the mock format exactly or dynamically stringified 
    ...
    const { error } = await supabase.from('protocols').insert([
      {
        ...
        status: 'Aberto',
        category: category,
        requester: user?.full_name || 'Usuário Autenticado'
      }
    ]);
        ...
        <div className="w-full max-w-[1200px] flex flex-col gap-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-800 pb-6">
            ...
            <div className="flex items-center w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
              <Step number={1} label="Tipo" status="completed" />
              <div className="w-12 h-0.5 bg-green-500 mx-3"></div>
              <Step number={2} label="Localização" status="active" />
              <div className="w-12 h-0.5 bg-slate-700 mx-3"></div>
              <Step number={3} label="Revisão" status="pending" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 min-h-[600px]">
            ...
            <div className="lg:col-span-5 flex flex-col gap-6 order-2 lg:order-1">
              <div className="bg-[#1c2632] rounded-xl p-6 shadow-sm border border-slate-700 flex flex-col gap-5">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-slate-300">Categoria de Acessibilidade</label>
                  <select
                  ...
                  >
                    <option value="Física">Física (Ex: Calçadas, Rampas)</option>
                    <option value="Visual">Visual (Ex: Pisos Táteis, Semáforos)</option>
          ...
          <div className="sticky bottom-0 bg-[#101922] pt-4 pb-8 border-t border-slate-800 mt-4 flex items-center justify-between lg:justify-end gap-4 z-40">
            <button onClick={() => navigate(-1)} className="px-6 py-3 rounded-lg border border-slate-700 text-slate-200 font-bold hover:bg-[#283039] transition-colors w-full lg:w-auto">
              Cancelar
            </button>
            <button
            ...
            >
              {loading ? <Loader2 size={16} className="animate-spin" /> : 'Finalizar Solicitação'}

...
function totalsFor(entries: FinanceEntry[]) {
  ...
  const pendingRevenue = entries.filter(item => item.type === 'revenue' && item.status !== 'paid').reduce((sum, item) => sum + item.amount, 0);
  const pendingCosts = entries.filter(item => item.type === 'cost' && item.status !== 'paid').reduce((sum, item) => sum + item.amount, 0);
  ...
  const closedProfit = closedRevenue - closedCosts;

  return {
    revenue,
    costs,
    pending: pendingRevenue + pendingCosts,
    pendingRevenue,
    pendingCosts,
    profit,
    balance: profit,
    closedProfit,
    closedBalance: closedProfit,
  };
}
...
export function Finance() {
  ...
  const [entryModalType, setEntryModalType] = useState<EntryType | null>(null);
  ...
  const openEntryModal = (type: EntryType, title = '') => {
    const category = finance.categories.find(item => item.type === 'both' || item.type === type)?.name ?? '';
    const person = finance.people[0] ?? '';

    setEntryDraft({
      ...defaultEntryDraft(selectedMonth),
      type,
      title,
      category,
      person,
      status: type === 'revenue' ? 'pending' : 'pending',
    });
  ...
  const updateEntryDraftMonth = (monthKey: string) => {
    const day = entryDraft.dueDate.slice(-2) || '15';
    setEntryDraft(prev => ({ ...prev, dueDate: `${monthKey}-${day}` }));
  };
  ...
  const handleEntrySubmit = (event: FormEvent) => {
    ...
    const entryMonth = getMonthKey(entry.dueDate);
    setSelectedMonth(entryMonth);
    setActiveTab('details');
    setEntryDraft(defaultEntryDraft(entryMonth));
    closeEntryModal();
  };
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[#080d12]">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => openEntryModal('revenue', 'Faturamento mensal')}
              className="flex h-10 items-center gap-2 rounded-lg border border-white/8 bg-white px-4 text-sm font-bold text-slate-700 transition-colors hover:bg-green-500/10"
            >
              <BadgeDollarSign size={16} /> Faturamento mensal
            </button>
            <button
              type="button"
              onClick={() => openEntryModal('revenue')}
        ...
        <div className="mb-5 inline-flex rounded-lg border border-white/8 bg-white/90 p-1 shadow-sm">
          {[
            ['overview', WalletCards, 'Visao geral'],
            ['details', ReceiptText, 'Detalhes do mes'],
          ].map(([tab, Icon, label]) => (
            <button
              type="button"
              key={tab as string}
              onClick={() => setActiveTab(tab as FinanceTab)}
              className={`flex h-10 items-center gap-2 rounded-md px-4 text-sm font-black transition-colors ${activeTab === tab
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <FinanceStat
            ...
            detail={`${selectedMonthEntries.filter(entry => entry.type === 'revenue').length} entradas de receita no mes`}
          ...
          <FinanceStat
            ...
            value={BRL.format(selectedTotals.profit)}
            detail={`faturamento menos custos do mes`}
            icon={CircleDollarSign}
            tone={selectedTotals.profit >= 0 ? 'blue' : 'amber'}
          />
          <FinanceStat
            label="Pendente/projetado"
            value={BRL.format(selectedTotals.pendingCosts)}
            detail={`${selectedMonthEntries.filter(entry => entry.type === 'cost' && entry.status !== 'paid').length} custos em aberto`}
            icon={Clock3}
            tone="amber"
          />
        </div>
      ...
      {entryModalType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-lg border border-white/10 bg-white p-5 shadow-2xl">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase text-blue-700">
                  {entryDraft.type === 'revenue' ? 'Faturamento' : 'Custo'}
                </p>
                <h2 className="mt-1 text-xl font-black text-slate-950">
                  {entryDraft.type === 'revenue' ? 'Adicionar faturamento' : 'Novo custo'}
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Selecione o mes do lancamento antes de salvar.
                </p>
              </div>
              <button
                type="button"
                onClick={closeEntryModal}
                className="flex size-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-100"
                title="Fechar modal"
              >
                <X size={18} />
              </button>
            ...
            <form onSubmit={handleEntrySubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ['cost', ReceiptText, 'Custo'],
                  ['revenue', BadgeDollarSign, 'Faturamento'],
                ].map(([value, Icon, label]) => (
                  <button
                    type="button"
                    key={value as string}
                    onClick={() => {
                      setEntryModalType(value as EntryType);
                      setEntryDraft(prev => ({
                        ...prev,
                        type: value as EntryType,
                        category: finance.categories.find(category => category.type === 'both' || category.type === value)?.name ?? prev.category,
                    ...
                    className={`flex h-11 items-center justify-center gap-2 rounded-lg border text-sm font-bold ${entryDraft.type === value
                      ? 'border-blue-600 bg-blue-500/10 text-blue-700'
                      : 'border-slate-200 bg-white text-slate-500'
                      }`}
                  >
              ...
              <label className="block">
                ...
                <input
                  ...
                  placeholder={entryDraft.type === 'revenue' ? 'Ex: Faturamento loja' : 'Ex: Manutencao impressora'}
                  autoFocus
                />
              </label>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <label className="block">
                  <span className="mb-1.5 block text-xs font-semibold text-slate-500">Mes</span>
                  <input
                    type="month"
                    value={getMonthKey(entryDraft.dueDate)}
                    onChange={event => updateEntryDraftMonth(event.target.value)}
                    className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"
                  />
                </label>
                <label className="block">
              ...
              <label className="flex h-11 items-center justify-between rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700">
                Repetir como fixo
                <input
                  type="checkbox"
                  checked={entryDraft.isFixed}
              ...
              <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
                <button
                  type="button"
                  onClick={closeEntryModal}
                  className="h-10 rounded-lg border border-slate-200 px-4 text-sm font-bold text-slate-600 hover:bg-slate-100"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex h-10 items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-500"
                >
                  <Plus size={16} />
                  {entryDraft.type === 'revenue' ? 'Salvar faturamento' : 'Salvar custo'}
                </button>
              </div>
            </form>
          </div>

...
UPDATE public.protocols SET state_code = CASE
...
ALTER TABLE public.protocols DROP CONSTRAINT IF EXISTS ck_protocols_state_code;
ALTER TABLE public.protocols ADD CONSTRAINT ck_protocols_state_code CHECK (
...
CREATE INDEX IF NOT EXISTS ix_protocols_state_code ON public.protocols (state_code);

CREATE OR REPLACE FUNCTION public.set_protocol_state_code()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_protocols_state_code ON public.protocols;
CREATE TRIGGER trg_protocols_state_code
    BEFORE INSERT OR UPDATE OF address, state_code ON public.protocols
    FOR EACH ROW EXECUTE FUNCTION public.set_protocol_state_code();

DO $$
BEGIN
    IF to_regclass('public.daily_operational_report_protocols') IS NOT NULL THEN
        ALTER TABLE public.daily_operational_report_protocols
            ADD COLUMN IF NOT EXISTS state_code VARCHAR(2);

        UPDATE public.daily_operational_report_protocols
           SET state_code = upper(substring(address FROM '(AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO)[[:space:]]*$'))
         WHERE state_code IS NULL;

        UPDATE public.daily_operational_report_protocols details
        ...
        CREATE INDEX IF NOT EXISTS ix_daily_report_protocols_state_code
            ON public.daily_operational_report_protocols (state_code);
    END IF;
END $$;

CREATE TABLE IF NOT EXISTS public.server_state_permissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id TEXT NOT NULL REFERENCES public.users (id) ON DELETE CASCADE,
    state_code VARCHAR(2) NOT NULL CHECK (state_code IN (
        'AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG',
        'PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'
    )),
...
CREATE INDEX IF NOT EXISTS ix_server_state_permissions_user_id
    ON public.server_state_permissions (user_id);
...
ALTER TABLE public.server_state_permissions ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.server_state_permissions FROM anon, authenticated;

...
CREATE INDEX IF NOT EXISTS idx_administrative_audit_created_at
    ON public.administrative_audit_events (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_administrative_audit_actor
    ON public.administrative_audit_events (actor_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_administrative_audit_action
    ON public.administrative_audit_events (action, created_at DESC);

ALTER TABLE public.administrative_audit_events ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.administrative_audit_events FROM anon, authenticated;
